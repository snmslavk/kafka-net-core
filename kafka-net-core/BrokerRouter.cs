...
{
    /// <summary>
    /// <para>This class provides an abstraction from querying multiple Kafka servers for Metadata details and caching this data.</para>
    /// <para>
    ...
    public class BrokerRouter : IBrokerRouter
    {
        ...
        public BrokerRouter(KafkaOptions kafkaOptions)
        {
            ...
            foreach (var endpoint in _kafkaOptions.KafkaServerEndpoints)
            {
                ...
                _defaultConnectionIndex.AddOrUpdate(endpoint, _ => conn, (_, __) => conn);
            }
        ...
        public BrokerRoute SelectBrokerRoute(string topic, int partitionId)
        {
            ...
            var topicMetadata = cachedTopic[0];

            var partition = topicMetadata.Partitions.Find(x => x.PartitionId == partitionId);
        ...
        public List<Topic> GetTopicMetadata(params string[] topics)
        {
            ...
            if (topicSearchResult.Missing.Count > 0)
            {
                ...
                RefreshTopicMetadata(topicSearchResult.Missing.Where(x => !_topicIndex.ContainsKey(x)).ToArray());
        ...
        private void UpdateInternalMetadataCache(MetadataResponse metadata)
        {
            ...
            foreach (var topic in metadata.Topics)
            {
                ...
                _topicIndex.AddOrUpdate(topic.Name, _ => localTopic, (_, __) => localTopic);
            }
        }
